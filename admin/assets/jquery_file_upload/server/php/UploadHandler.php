...
class UploadHandler
{
    ...
    protected $error_messages = array(
        1 => 'Размер загружаемого файла привышает значение upload_max_filesize в настройках php.ini',
        2 => 'Размер загружаемого файла привышает значение в указаниях формы HTML',
        3 => 'Файл загружен частично',
        4 => 'Файл не был загружен',
        6 => 'Временная папка не найдена',
        7 => 'Не удалось записать файл на диск',
        8 => 'Расширение PHP остановило загрузку файлов',
        'post_max_size' => 'Размер загружаемого файла привышает значение upload_max_filesize в настройках php.ini',
        'max_file_size' => 'Размер файла слишком велик',
        'min_file_size' => 'Размер файла слишком мал',
        'accept_file_types' => 'Запрещенный формат файла',
        'max_number_of_files' => 'Преавышено максимальное число файлов для загрузки',
        'max_width' => 'Размер изображения превышает максимальную ширину',
        'min_width' => 'Размер изображения менее минимальной ширины',
        'max_height' => 'Размер изображения превышает максимальную высоту',
        'min_height' => 'Размер изображения менее минимальной высоты'
    );
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file = new stdClass();
        if($this->options["generate_name"]) {
            $file->name = $this->rndstring().".".$this->getExtension($name);
        } else {
            $file->name = $name;
        }
    ...
    public function post($print_response = true) {
        ...
        if($this->options['callback_done'] != NULL) {
            foreach ($files as $key => $file) {
                $files[$key] = $this->options['callback_done']($file);
            }
        }
